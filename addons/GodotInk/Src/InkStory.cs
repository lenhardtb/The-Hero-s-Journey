...
public partial class InkStory : Resource
{
	...
	{
		runtimeStory.variablesState.SetGlobal(variableName,Ink.Runtime.Value.Create(FromVariant(value)));
	...
	public int IncrementVariable(string variableName)
	{
		int newValue = (int)ToVariant(runtimeStory.variablesState[variableName]) + 1;
		runtimeStory.variablesState[variableName] = FromVariant(newValue);
		return newValue;
	}
	/// <summary>
	///
	...
	{
		GD.Print("in InkStory.LoadState, is jsonstate null? " + (jsonState == null));
	...
	//Added by Bo Lenhardt 3/1/24
	public void ChangeStoryKeepVariables(string rawStory)
	{
		Ink.Runtime.VariablesState varState = runtimeStory.variablesState;
		RawStory = rawStory;//reset state, use new story text
		//repopulate from previous variablesState
		foreach(string name in varState)
		{
			runtimeStory.variablesState.SetGlobal(name, Ink.Runtime.Value.Create(varState[name]));
		}
		foreach(string name in internalObservers.Keys)
		{
			runtimeStory.ObserveVariable(name, internalObservers[name]);
		}
	}
